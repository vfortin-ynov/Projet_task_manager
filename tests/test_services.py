...
class TestEmailService:
    ...
    def test_connect_success(self, mock_smtp_constructor):
        ...
        # Vérifier que le constructeur SMTP est appelé avec les bons arguments, y compris le timeout
        mock_smtp_constructor.assert_called_once_with("smtp.gmail.com", 587, timeout=5)
        self.mock_smtp.starttls.assert_called_once()
        self.mock_smtp.login.assert_called_once_with("user@example.com", "password123")

    @patch('smtplib.SMTP')
    def test_connect_failure(self, mock_smtp_constructor):
        """Test d'échec de connexion au serveur SMTP."""
        # Configurer le mock pour lever une exception lors de l'appel à starttls()
        self.mock_smtp.starttls.side_effect = smtplib.SMTPException("Erreur SMTP")
        mock_smtp_constructor.return_value = self.mock_smtp
        
        # Vérifier que l'exception est levée avec le bon message
        with pytest.raises(ConnectionError, match="Échec de la connexion SMTP"):
            self.email_service.connect("user@example.com", "password123")
        ...
        mock_smtp_constructor.assert_called_once_with("smtp.gmail.com", 587, timeout=5)
        self.mock_smtp.starttls.assert_called_once()
        self.mock_smtp.login.assert_not_called()  # Ne devrait pas être appelé en cas d'échec de starttls
    ...
    def test_send_email_success(self, mock_smtp_constructor):
        ...
        result = self.email_service.send_email("recipient@example.com", "Sujet", "Corps du message")
    ...
    def test_send_task_reminder(self, mock_smtp_constructor):
        ...
        result = self.email_service.send_task_reminder("user@example.com", "Tâche importante", due_date)
        ...
        assert expected_date in body, f"Date attendue '{expected_date}' non trouvée dans le corps de l'email: {body}"

    def test_send_task_reminder_invalid_email(self):
        """Test d'envoi de rappel avec email invalide."""
        with pytest.raises(ValueError, match="Format d'email invalide"):
            self.email_service.send_task_reminder("invalid-email", "Tâche", datetime.now())

    @patch('smtplib.SMTP')
    def test_connect_smtp_exception(self, mock_smtp):
        """Test de l'échec de connexion avec exception SMTP"""
        mock_smtp.return_value.starttls.side_effect = smtplib.SMTPException("Erreur SMTP")
    ...
    def test_disconnect_with_error(self, mock_smtp):
        ...
        mock_connection.quit.side_effect = smtplib.SMTPException("Erreur de déconnexion")
        
        # Créer une instance de EmailService et définir manuellement la connexion
    ...
    def test_send_email_smtp_error(self, mock_smtp):
        ...
        self.email_service.connect("test@example.com", "password")
        mock_smtp.return_value.send_message.side_effect = smtplib.SMTPException("Erreur d'envoi")
    ...
    def test_send_task_reminder_invalid_email(self):
        ...
        with pytest.raises(ValueError):
            self.email_service.send_task_reminder("email-invalide", "Tâche", datetime.now())

    def test_send_completion_notification_invalid_email(self):
        """Test d'envoi de notification avec email invalide"""
        with pytest.raises(ValueError):
            self.email_service.send_completion_notification("email-invalide", "Tâche")
    ...
    def test_send_completion_notification_success(self, mock_smtp_constructor):
        ...
        assert "Tâche importante" in content, f"Le contenu du message ne contient pas le titre de la tâche: {content}"

    def test_validate_email_valid(self):
        """Test de validation d'email valide"""
        # Ne devrait pas lever d'exception
        self.email_service._validate_email("test@example.com")
        self.email_service._validate_email("user.name+tag@example.co.uk")
        self.email_service._validate_email("user@sub.domain.com")

    def test_validate_email_invalid(self):
        """Test de validation d'email invalide"""
        # Utiliser une expression régulière plus précise pour le message d'erreur
        with pytest.raises(ValueError, match=r"Format d'email invalide"):
            self.email_service._validate_email("invalid-email")
            
        with pytest.raises(ValueError, match=r"Format d'email invalide"):
            self.email_service._validate_email("user@")
    ...
    def test_connect_smtp_exception_during_login(self, mock_smtp_constructor):
        ...
        mock_smtp = MagicMock()
        mock_smtp_constructor.return_value = mock_smtp
        mock_smtp.login.side_effect = smtplib.SMTPException("Erreur d'authentification")
    ...
    def test_validate_email_edge_cases(self):
        ...
        # Test avec un email valide avec des espaces (doit être valide après strip)
        assert self.email_service._validate_email("  test@example.com  ") is True
        ...
        assert self.email_service._validate_email("test@sub.example.com") is True
    ...
    def test_connect_quit_failure(self, mock_smtp_constructor):
        ...
        mock_smtp = MagicMock()
        mock_smtp_constructor.return_value = mock_smtp
        mock_smtp.login.side_effect = smtplib.SMTPException("Erreur de connexion")
        mock_smtp.quit.side_effect = smtplib.SMTPException("Erreur de déconnexion")
...
class TestReportService:
    ...
    def test_export_tasks_csv(self, mock_file):
        ...
        result = self.report_service.export_tasks_csv(self.tasks, 'test_export.csv')
        ...
        assert 'id,title,description,priority,status,created_at,completed_at,project_id' in header_call

    def test_export_tasks_csv_empty_list(self):
        """Test d'exportation avec une liste vide de tâches."""
        with patch('builtins.open') as mock_file:
            result = self.report_service.export_tasks_csv([], 'empty_export.csv')
    ...
    def test_export_tasks_csv_io_error(self, mock_file):
        ...
        result = self.report_service.export_tasks_csv(self.tasks, 'error_export.csv')
        assert result is False
    ...
    def test_export_tasks_csv(self, tmp_path):
        """Test d'exportation des tâches en CSV"""
        tasks = [
        ...
        filename = tmp_path / "tasks.csv"
        self.report_service.export_tasks_csv(tasks, str(filename))
