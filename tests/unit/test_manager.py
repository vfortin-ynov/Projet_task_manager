...
class TestTaskManagerBasics:
    ...
    def test_get_tasks_by_priority(self, manager_with_tasks):
        ...
        medium_priority = manager_with_tasks.get_tasks_by_priority(
            Priority.MEDIUM
        )
        ...
        assert all(
            task.priority == Priority.MEDIUM for task in medium_priority
        )
    ...
    def test_filter_tasks(self, manager_with_tasks):
        ...
        high_todo = manager_with_tasks.filter_tasks(
    ...
    def test_update_task_with_string_priority(self, empty_manager):
        """Test de mise à jour d'une tâche avec une
        priorité sous forme de chaîne"""
    ...
    def test_get_stats_is_alias_for_get_statistics(self, empty_manager):
        ...
        assert (
            stats_method is not statistics_method
        )  # They are different method objects
    ...
    def test_update_task_with_nonexistent_priority_string(self, empty_manager):
        """Test de mise à jour avec une chaîne de priorité
        qui n'existe pas"""
        task_id = empty_manager.add_task("Tâche de test")

        # Vérifie que la méthode lève une KeyError
        # pour une priorité inexistante
        with pytest.raises(KeyError):
            empty_manager.update_task(
                task_id, priority="NON_EXISTENT_PRIORITY"
            )
...
class TestTaskManagerFiltering:
    ...
    def test_filter_tasks_combines_filters(self, manager_with_tasks):
        ...
        result = manager_with_tasks.filter_tasks(
            status=Status.TODO, priority=Priority.HIGH, project_id="PROJ-1"
        )
        assert len(result) == 1
        task = result[0]
    ...
    def test_filter_tasks_by_project_id(self, empty_manager):
        ...
        # Modification directe pour ajouter un project_id
    ...
    def test_filter_tasks_missing_project_id(self, empty_manager):
        ...
        # Filtrage par project_id - ne devrait retourner
        # que la tâche avec project_id
        filtered = empty_manager.filter_tasks(project_id="PROJ1")
        assert len(filtered) == 1
...
class TestTaskManagerPersistence:
    ...
    @patch("json.dump")
    def test_save_to_file_handles_io_error(
        self, mock_json_dump, mock_file, manager_with_tasks
    ):
    ...
    def test_load_from_nonexistent_file_creates_empty_manager(self, tmp_path):
        """Vérifie que le chargement d'un fichier inexistant crée
    ...
    @patch(
        "builtins.open",
        side_effect=json.JSONDecodeError("Erreur de décodage", "{}", 0),
    )
...
class TestTaskManagerEdgeCases:
    ...
    def test_filter_tasks_none_parameters(self, manager_with_tasks):
        ...
        assert len(manager_with_tasks.filter_tasks(priority=None)) == len(
            all_tasks
        )
        assert len(manager_with_tasks.filter_tasks(status=None)) == len(
            all_tasks
        )
