...
class TestTaskWorkflow:
    ...
    def test_complete_task_workflow(self, manager_with_temp_file):
        ...
        new_manager.storage_file = (
            manager.save_file
        )  # S'assurer que storage_file est également défini

        # Vérifier que le fichier existe avant de le charger
        assert os.path.exists(
            manager.save_file
        ), f"Le fichier {manager.save_file} n'existe pas"
        ...
        assert (
            loaded_task is not None
        ), "La tâche n'a pas été chargée depuis le fichier"
        assert (
            loaded_task.title == "Tâche de test"
        ), "Le titre de la tâche chargée est incorrect"
        assert loaded_task.status == Status.IN_PROGRESS, (
            f"Le statut de la tâche chargée est incorrect: "
            f"{loaded_task.status} au lieu de {Status.IN_PROGRESS.name}"
        )
        ...
        assert os.path.exists(
            new_manager.save_file
        ), f"Le fichier {new_manager.save_file} n'existe plus \
        après la sauvegarde"
        ...
        with open(new_manager.save_file, "r") as f:
            ...
            assert (
                len(updated_data) > 0
            ), "Le fichier ne contient aucune tâche après la mise à jour"
            updated_task = next(
                (t for t in updated_data if t["id"] == task_id), None
            )
            assert (
                updated_task is not None
            ), "La tâche mise à jour n'est pas dans le fichier"
            assert updated_task["status"] == Status.DONE.name, (
                f"Le statut de la tâche dans le fichier n'est pas à jour: "
                f"{updated_task['status']} au lieu de {Status.DONE.name}"
            )
    ...
    def test_task_filtering_workflow(self, manager_with_temp_file):
        ...
        manager.update_task(
            task1_id, status=Status.TODO, priority=Priority.LOW
        )

        task2_id = manager.add_task("Tâche 2", "")
        manager.update_task(
            task2_id, status=Status.IN_PROGRESS, priority=Priority.HIGH
        )

        task3_id = manager.add_task("Tâche 3", "")
        manager.update_task(
            task3_id, status=Status.DONE, priority=Priority.HIGH
        )
