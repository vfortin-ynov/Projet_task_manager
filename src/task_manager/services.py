...
class EmailService:
    ...
    def connect(self, username: str, password: str) -> None:
        ...
        try:
            self.connection = smtplib.SMTP(
                self.smtp_server, self.port, timeout=5
            )
    ...
    def send_task_reminder(
        self, email: str, task_title: str, due_date: datetime
    ) -> bool:
...
class ReportService:
    ...
    def generate_daily_report(
        self, tasks: List[Task], date: Optional[datetime] = None
    ) -> Dict[str, Any]:
        ...
        daily_tasks = [
            ...
            for task in tasks
            if task.created_at
            and datetime.fromisoformat(task.created_at).date()
            == report_date.date()
        ]
        ...
        return {
            ...
            "completed_tasks": sum(
                1 for t in daily_tasks if t.status.name == "DONE"
            ),
        }
