...
def main():
    ...
    task1 = task_manager.add_task(
        "Faire les courses", "Acheter du lait et des œufs", Priority.HIGH
    )
    ...
    task3 = task_manager.add_task(
        "Faire du sport", "30 minutes de course", Priority.LOW
    )
    task4 = task_manager.add_task(
        "Préparer la réunion", "Préparer les slides pour demain", Priority.HIGH
    )
    ...
    print(
        f"Tâches marquées comme terminées : {task_manager.get_task(task1).title}, {task_manager.get_task(task3).title}\n"
    )
    ...
    print(
        f"Tâches complétées : {stats['completed']} ({(stats['completed']/stats['total'])*100:.1f}%)"
    )
